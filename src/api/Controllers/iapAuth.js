...
const performAuth = asyncErrorWrapper(async (req, res, next) => {
    ...
    try {
        if (!isJWTTokenIncluded(req)) {
            //checks if token included, returns token or thorws error
        ...
        console.log(
            "decoded_iap_jwt.payload.gcip.domains --> ",
            decoded_iap_jwt.payload["gcip"]["domains"]
        );

        if (
            !decoded_iap_jwt.payload["gcip"]["domains"].includes(
